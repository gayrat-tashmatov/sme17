...
export function ModuleCardGrid({ modules, variant, columns = 3 }: ModuleCardGridProps) {
  ...
  /**
   * Only the 6 priority modules from PF-50 Article 4 get the "До 01.07.2026"
   * deadline badge. New modules from international benchmarks and queue modules
   * show their own phase metadata (typically 2-я половина 2026 / 2027).
   */
  ...
  return (
    <div className={grid}>
      {modules.map((m, i) => (
        ...
        >
          <Link href={m.href} className="block h-full focus-ring rounded-xl relative">
            {isNew && (
            ...
            <Card hover className={cn(
              ...
              <div className="mt-5 flex items-center justify-between">
                <Badge variant={isPriority ? 'priority' : 'queue'}>
                  {t(isPriority ? 'ui.priority-deadline' : m.phase)}
                </Badge>
